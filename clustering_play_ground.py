...
def prepare():
    ...
    dim_recud1 = alg_to_partial(PCA) + alg_to_partial(TruncatedSVD) + alg_to_partial(KernelPCA,
                                                                                     kernel='poly') + alg_to_partial(
        NMF)
    dim_recud2 = alg_to_partial(TSNE, learning_rate='auto', init='pca', k1='perplexity',
                                l1=[30, 45, 55]) + alg_to_partial(Isomap)
...
def alg_to_partial(alg, k1=None, l1=None, **kwargs):
    ...
    inner_kwargs['random_state'] = 42
    if 'Iso' in str(alg):
        inner_kwargs.pop('random_state')
    if k1 is not None:
        for ll in l1:
            inner_kwargs[k1] = ll
            algs.append(partial(alg, **inner_kwargs))
    else:
        algs = [partial(alg, **inner_kwargs)]
    return algs
...
def f(xxxx, points, slc_to_seg, slice_to_feature_source, slice_to_feature_target, voxel_spacing):
    ...
    for i in range(n_clus):
        ...
        sc_mean, sc_var = (
        np.mean(source_clusters[i_source], axis=0), np.mean(np.std(source_clusters[i_source], axis=0)))
        ...
        sdice1 = sdice(np.expand_dims(tc_mean > 0.5, axis=0), np.expand_dims(sc_mean > 0.5, axis=0), voxel_spacing, 1,
                       for_clcc=True)
        vars_sc.append(sc_var)
        vars_tc.append(tc_var)
        sdices.append(sdice1)
    print(f'{da1},{da2},{clus_a}, {n_clus},{d1},{d2}',
          (float(np.mean(vars_tc)), float(np.mean(vars_sc)), float(np.mean(sdices))))
    return f'{da1},{da2},{clus_a}, {n_clus},{d1},{d2}', (
    float(np.mean(vars_tc)), float(np.mean(vars_sc)), float(np.mean(sdices)))


def main():
    points, slc_to_seg, all1, slice_to_feature_source, slice_to_feature_target = prepare()
    f1 = partial(f, slice_to_feature_source=slice_to_feature_source, slice_to_feature_target=slice_to_feature_target,
                 voxel_spacing=(1, 0.95, 0.95), slc_to_seg=slc_to_seg, points=points)
    resres = []
    with Pool(processes=4) as pool:
        for i in tqdm(range(0, len(all1), 4),total=len(all1)//4):
